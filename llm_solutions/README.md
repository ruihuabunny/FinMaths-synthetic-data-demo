...
`public/submission.schema.json` 为准，它们会完整发送给 LLM。当前最小 `public/prompt.md`
只做路由；真正的数学与数值合同由 solver 在运行时调用
`query_greeks_task_contract_v1` 获得，权限和预算以同时发送的
`public/runtime_contract.json` 为准。
...
Runner 的 system/user 文本用于指导模型写代码，不替代 package 的四个 public source of
truth。源码仍必须在受限 harness 中查询合同和 inputs；runner 不把 repo-side analytic
solver、reference artifact 或 verifier source注入 evaluation prompt。
...
新的输出目录运行你已经 materialize 并审核过的当前-interface batch：

```bash
.venv/bin/python llm_solutions/run_bsm_market_implied_greeks.py \
  --run-root /absolute/path/to/current-interface/packages \
