...
> 当前分支状态（2026-08-12）：本文件是 future/design-only 计划，不是当前可运行 task。
...
> Git-ignored `runs/f2a_v5/` 中可存在其他分支/实验的本地 pilot 文件；它们不对应本文件的
> frictionless F2A 合同，也不构成当前 source tree 的可调用实现或发布制品。
