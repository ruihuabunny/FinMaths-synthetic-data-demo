...
from synthetic_derivatives.packaging_analytic_and_implied_greeks_iv.trajectory import reference_trajectory
...
def _verifier_sources() -> dict[str, str]:
    return {
        "__init__.py": '''"""Package-local trusted verifier."""
''',
...
''',
        "test_contract.py": '''from verifier.runtime import MarketGreeksSubmission
...
The verifier reconstructs the canonical answer from `public/task.duckdb` and
`verifier/oracle_config.json`. It does not read the packaged reference answer.
''',
    }


def _write_verifier(package_root: Path, repository_root: Path) -> None:
...
def _validate_manifest_shape(manifest: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or manifest["task_version"] != BSM_MARKET_GREEKS_TASK_VERSION
...
def build_bsm_greeks_package(
    *,
    ...
    source_solver = repository / "src/synthetic_derivatives/packaging_analytic_and_implied_greeks_iv/reference_solver.py"
...
def verify_bsm_greeks_package(
    ...
    *,
    ...
    expected_solver_interface_digest = bsm_market_greeks_solver_interface_digest(
        ...
        runtime_contract=runtime,
        submission_schema_bytes=(
            root / "public/submission.schema.json"
        ).read_bytes(),
    )
    inputs = load_bsm_greeks_inputs(database)
    expected_task_id = stable_package_task_id(
        parent_logical_checksum=manifest["parent_snapshot"]["logical_checksum"],
        valuation_date=date.fromisoformat(manifest["valuation_date"]),
        selected_underlyings=manifest["selected_underlying_ids"],
        solver_interface_digest=expected_solver_interface_digest,
    )
    if manifest["task_id"] != expected_task_id:
        raise ValueError("task identity is not bound to the solver interface")
