...
def market_greeks_method_contract() -> dict[str, Any]:
    ...
    return {
        ...
        "filtration": analytic["filtration"],
        "conditioning_information": (
            "valuation_date_spot_contract_flat_public_curves_and_visible_quote"
        ),
        "valuation_timestamp": analytic["valuation_timestamp"],
        "model": analytic["model"],
        "payoff": analytic["payoff"],
        "exercise_style": analytic["exercise_style"],
        "settlement_type": analytic["settlement_type"],
        "rate_curve": analytic["rate_curve"],
        "dividend_curve": analytic["dividend_curve"],
        ...
        "analytic_operation_order_id": analytic["operation_order_id"],
        "analytic_operation_sequence": analytic["operation_sequence"],
        "analytic_operation_formulas": analytic["operation_formulas"],
        "greeks": {
        ...
        "input_units": {
            "spot": analytic["input_units"]["spot"],
            "strike": analytic["input_units"]["strike"],
            "tau": analytic["input_units"]["tau"],
            "r": analytic["input_units"]["r"],
            "q": analytic["input_units"]["q"],
...
def bsm_market_greeks_solver_interface_digest(
    *,
    prompt: str,
    runtime_contract: Mapping[str, Any],
    submission_schema_bytes: bytes,
) -> str:
    """Bind every solver-visible instruction surface into one stable digest."""

    if not isinstance(prompt, str) or not prompt.endswith("\n"):
        raise ValueError("solver prompt must be non-empty newline-terminated text")
    if not isinstance(runtime_contract, Mapping):
        raise ValueError("runtime contract must be a mapping")
    if not isinstance(submission_schema_bytes, bytes) or not submission_schema_bytes:
        raise ValueError("submission schema must be non-empty bytes")
    return digest_json(
        {
            ...
            "submission_schema_digest": sha256(
...
def validate_package_config(raw: Mapping[str, Any]) -> None:
    ...
    fixed = {
        "package_config_version": BSM_MARKET_GREEKS_PACKAGE_CONFIG_VERSION,
        "variant_id": BSM_MARKET_GREEKS_VARIANT_ID,
        "task_family": "bsm_greeks",
        "task_version": BSM_MARKET_GREEKS_TASK_VERSION,
