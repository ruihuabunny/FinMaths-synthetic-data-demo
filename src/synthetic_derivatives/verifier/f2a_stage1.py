"""Canonical Stage-1 physical-path estimator for F2A v5.
...
def diffusion_fisher_information(
    diffusion_node_values: Sequence[float],
    q_rows: Sequence[Sequence[Sequence[float]]],
) -> tuple[tuple[float, ...], ...]:
    """Expected Gaussian diffusion information used for scaling diagnostics."""

    beta = tuple(float(value) for value in diffusion_node_values)
    size = len(beta)
    result = _matrix(size, size)
    for q_row in q_rows:
        q_beta = _matvec(q_row, beta)
        variance = sum(beta[index] * q_beta[index] for index in range(size))
        if variance <= 0.0:
            raise ValueError("Fisher information requires positive interval variance")
        for i in range(size):
            for j in range(size):
                result[i][j] += (
                    2.0 * q_beta[i] * q_beta[j] / (variance * variance)
                )
    return tuple(tuple(value for value in row) for row in result)


def diffusion_fisher_covariance(
    diffusion_node_values: Sequence[float],
    q_rows: Sequence[Sequence[Sequence[float]]],
) -> tuple[tuple[float, ...], ...]:
    information = diffusion_fisher_information(diffusion_node_values, q_rows)
    eigenvalues = _symmetric_eigenvalues(information)
    if not eigenvalues or eigenvalues[0] <= 0.0:
        raise ValueError("diffusion Fisher information is not positive definite")
    return tuple(tuple(value for value in row) for row in _inverse(information))
