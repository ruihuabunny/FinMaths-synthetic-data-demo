...
_DATABASE_SCHEMA_VERSION = "bsm-greeks-task-duckdb-v1.0.0"
_VARIANT_ID = "bsm_market_implied_greeks_v1"
_TASK_VERSION = "1.0.0"
_METHOD_ID = "bsm-mid-iv-bisection80-analytic-greeks-v1"
_SUBMISSION_SCHEMA_VERSION = "bsm-market-implied-greeks-submission-v1.0.0"
_SELECTION_POLICY_ID = (
    "two-nearest-expiries-five-abs-log-forward-moneyness-pairs-v1"
)
_LOGICAL_CHECKSUM_ID = "sha256-bsm-greeks-canonical-logical-rows-v1"
...
def _load_method_contract(
    ...
    rows = connection.execute(
        "SELECT task_id, method_id, contract_json "
        "FROM solver_visible.greeks_task_contract"
    ).fetchall()
    if len(rows) != 1 or rows[0][1] != _METHOD_ID:
        raise ValueError("database has an invalid market-Greeks method row")
    contract = _json(rows[0][2])
    if not isinstance(contract, dict) or contract.get("contract_id") != _METHOD_ID:
        raise ValueError("database has an invalid market-Greeks method contract")
    return contract
...
def _validate_public_connection(connection: duckdb.DuckDBPyConnection) -> None:
    ...
    method_rows = connection.execute(
        "SELECT task_id FROM solver_visible.greeks_task_contract"
    ).fetchall()
    if method_rows != [(meta[1],)]:
        raise ValueError("BSM Greeks method task identity is invalid")
    contract = _load_method_contract(connection)
    if (
        contract.get("variant_id") != _VARIANT_ID
        or contract.get("pricing_measure_id") != _RISK_NEUTRAL_MEASURE_ID
        or contract.get("numeraire_id") != _NUMERAIRE_ID
        or contract.get("day_count") != "Actual365Fixed"
        or contract.get("pricing_law")
        != "exact_analytic_constant_parameter_european_bsm"
        or contract.get("output_precision") != 8
        or contract.get("rounding") != "ROUND_HALF_EVEN"
        or contract.get("success_iv_status") != _SUCCESS_IV_STATUS
    ):
        raise ValueError("BSM Greeks method contract changed")
    inversion = contract.get("iv_inversion")
    if not isinstance(inversion, dict) or (
        inversion.get("method_id") != "bsm-bisection-float64-80-v1"
        or inversion.get("volatility_bracket")
        != [_LOWER_VOLATILITY, _UPPER_VOLATILITY]
        or inversion.get("iterations") != _BISECTION_ITERATIONS
        or inversion.get("early_stop") is not False
        or inversion.get("fallback_method") is not None
    ):
        raise ValueError("BSM Greeks IV method contract changed")
