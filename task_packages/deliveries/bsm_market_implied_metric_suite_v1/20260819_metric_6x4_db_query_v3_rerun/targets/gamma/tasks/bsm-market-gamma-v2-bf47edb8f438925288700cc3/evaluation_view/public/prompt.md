# Task: Market-implied BSM unit Gamma

For every public option quote, recover its market-implied volatility internally and compute only unit Gamma.
...
- Target field: `unit_gamma`.
- Required method ID: `bsm-mid-iv-bisection80-analytic-gamma-v1`.
- Reported as the Delta change per +1.00 change in spot.
- Each result row must contain exactly `row_id` and `unit_gamma`.
