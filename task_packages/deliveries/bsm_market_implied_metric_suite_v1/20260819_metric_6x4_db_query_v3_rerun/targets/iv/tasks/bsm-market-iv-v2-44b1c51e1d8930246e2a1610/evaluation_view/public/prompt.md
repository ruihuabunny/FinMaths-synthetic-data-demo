# Task: Market-implied BSM market-implied volatility

For every public option quote, recover its market-implied volatility.
...
- After all 80 updates, use the unrounded binary64 root as the result before final canonicalization.
...
- Target field: `market_implied_volatility`.
- Required method ID: `bsm-mid-iv-bisection80-v1`.
- Annualized volatility expressed as a decimal; 0.20 means 20%.
- Each result row must contain exactly `row_id`, `iv_status` set to `CONVERGED_FIXED_ITERATIONS`, and `market_implied_volatility`.
