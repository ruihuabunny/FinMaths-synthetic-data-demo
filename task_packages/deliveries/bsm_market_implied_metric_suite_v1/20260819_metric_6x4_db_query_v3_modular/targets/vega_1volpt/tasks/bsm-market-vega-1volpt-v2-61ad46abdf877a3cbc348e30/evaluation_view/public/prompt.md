# Task: Market-implied BSM unit Vega for one volatility point

For every public option quote, recover its market-implied volatility internally and compute only unit Vega for one volatility point.
...
- Target field: `unit_vega_1volpt`.
- Required method ID: `bsm-mid-iv-bisection80-analytic-vega-1volpt-v1`.
- Reported for a +0.01 absolute change in volatility.
- Each result row must contain exactly `row_id` and `unit_vega_1volpt`.
