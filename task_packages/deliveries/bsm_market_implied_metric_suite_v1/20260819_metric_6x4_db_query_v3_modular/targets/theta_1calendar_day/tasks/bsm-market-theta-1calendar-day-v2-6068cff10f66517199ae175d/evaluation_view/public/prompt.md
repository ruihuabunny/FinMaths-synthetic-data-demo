# Task: Market-implied BSM unit Theta for one calendar day

For every public option quote, recover its market-implied volatility internally and compute only unit Theta for one calendar day.
...
- Target field: `unit_theta_1calendar_day`.
- Required method ID: `bsm-mid-iv-bisection80-analytic-theta-1calendar-day-v1`.
- Reported for one calendar day passing with expiry fixed.
- Each result row must contain exactly `row_id` and `unit_theta_1calendar_day`.
